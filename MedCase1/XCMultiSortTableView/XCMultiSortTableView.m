...
@interface XCMultiTableView () <UITableViewDelegate, UITableViewDataSource, UIScrollViewDelegate,PickerViewControllerDelegate,AgePickerViewControllerDelegate>
...
@property (strong,nonatomic) UILongPressGestureRecognizer *longPressContentTableViewCell;
...
- (id)initWithFrame:(CGRect)frame
{
    ...
    if (self) {
        ...
        //contentTableView.separatorColor = [UIColor greenColor];
...
- (void)setDatasource:(id<XCMultiTableViewDataSource>)datasource_ {
    if (datasource != datasource_) {
        ...
        responseGestureEnable = [datasource_ respondsToSelector:@selector(gestureEnable)];
        [self reset];
    }
}
...
//        }
        label.center = CGPointMake(cellW / 2.0f, cellH / 2.0f);
        label.bounds = CGRectMake(0, 0, cellW-5, cellH-5);
        label.tag = i;
//        if(i == 0){
//      
//        }else {
            UITapGestureRecognizer *labelTaped = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(labelTaped:)];
            
            label.userInteractionEnabled = YES;
            [label addGestureRecognizer:labelTaped];
            
            self.longPressContentTableViewCell = [[UILongPressGestureRecognizer alloc] initWithTarget:self action:@selector(longPresssContentTableViewCell:)];
            
            [label addGestureRecognizer:self.longPressContentTableViewCell];
...
{
    BOOL gestureIsEnabled = responseGestureEnable ? [self.datasource gestureEnable] : NO;
    if(gestureIsEnabled){
        if([recognizer.view isKindOfClass:[UILabel class]]){
            ...
            CGRect frame = [selectedLabel convertRect:recognizer.view.frame toView:self.superview];
            switch (viewTag) {
                case 0:{
                ...
                case 1:{
                    UIStoryboard *myStoryboard = [UIStoryboard storyboardWithName:@"ModelManagement" bundle:nil];
                    UINavigationController *nav = [myStoryboard instantiateViewControllerWithIdentifier:@"AgePickerNavigation"];
                    AgePickerViewController *agePicker = (AgePickerViewController*)[[nav viewControllers] objectAtIndex:0];
                    agePicker.ageDelegate = self;
                    if([selectedLabel.text containsString:@"-"]){
                        agePicker.defaultString = self.selectedLabel.text;
                    }
                    
                    UIPopoverController *popover = [[UIPopoverController alloc] initWithContentViewController:nav];
                    [popover setPopoverContentSize:CGSizeMake(400, 400)];
                    [popover presentPopoverFromRect:frame inView:self.superview permittedArrowDirections:UIPopoverArrowDirectionAny animated:YES];
                ...
                case 3:{
                    ...
                    UIStoryboard *myStoryboard = [UIStoryboard storyboardWithName:@"ModelManagement" bundle:nil];
                    UINavigationController *nav = [myStoryboard instantiateViewControllerWithIdentifier:@"PickerNavigation"];
                    PickerViewController *pickerVC = (PickerViewController*)[[nav viewControllers] objectAtIndex:0];
                    pickerVC.PickerVCDelegate = self;
                    pickerVC.defaultPickerString = [self.selectedLabel.text stringByAppendingString:@","];
...
-(void)selectedAgeRangeIs:(NSString *)ageString
{
    ...
    UIView *tempView = self.selectedLabel.superview.superview.superview;
    if([tempView isKindOfClass:[UITableViewCell class]]){
        UITableViewCell *tempCel = (UITableViewCell*)tempView;
        indexPath = [contentTableView indexPathForCell:tempCel];
    }
    
    NSMutableArray *ary = [[contentDataArray objectAtIndex:indexPath.section] objectAtIndex:indexPath.row];
    
    ary[self.selectedLabel.tag] = ageString;
    
    [contentTableView reloadRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
}
#pragma mask - picker view controller delegate
-(void)getSelectedItems:(NSArray *)items withString:(NSString *)str
{
    ...
    ary[self.selectedLabel.tag] = str;
...
{
    if(action == @selector(insertRowFromUp) || action == @selector(insertRowFromDown) || action == @selector(deleteThisRow)){
        return YES;
    }
    return NO;
}
-(void)longPresssContentTableViewCell:(UILongPressGestureRecognizer*)recognizer
{
    BOOL gestureIsEnabled = responseGestureEnable ? [self.datasource gestureEnable] : NO;
    if(gestureIsEnabled){
        if (recognizer.state == UIGestureRecognizerStateBegan){
            [self becomeFirstResponder];
            UIView *labelSuperView = (UIView*)recognizer.view.superview;
            UIView *yy = labelSuperView.superview;
            if([yy.superview isKindOfClass:[UITableViewCell class]]){
                NSIndexPath *temp = [contentTableView indexPathForCell:(UITableViewCell*)yy.superview];
                self.currentIndexPath = temp;
            }
            UIMenuItem *insertUp = [[UIMenuItem alloc] initWithTitle:@"从上插入行" action:@selector(insertRowFromUp)];
            UIMenuItem *insertDown = [[UIMenuItem alloc] initWithTitle:@"从下插入行" action:@selector(insertRowFromDown)];
            UIMenuItem *deleteRow = [[UIMenuItem alloc] initWithTitle:@"删除行" action:@selector(deleteThisRow)];
            
            UIMenuController *menu = [UIMenuController sharedMenuController];
            [menu setMenuItems:[NSArray arrayWithObjects:insertDown,insertUp,deleteRow, nil]];
            CGRect frame = [recognizer.view convertRect:recognizer.view.frame toView:self.superview];
            frame.origin.y -= 44;
...
-(void)insertRowFromUp
{
    ...
    NSIndexPath *indexPath;
    if(self.currentIndexPath.row - 1 < 0){
        indexPath = [NSIndexPath indexPathForRow:self.currentIndexPath.row inSection:self.currentIndexPath.section];
    }else {
        indexPath = [NSIndexPath indexPathForRow:self.currentIndexPath.row inSection:self.currentIndexPath.section];
    }
    [[leftHeaderDataArray objectAtIndex:self.currentIndexPath.section] insertObject:tempArray atIndex:indexPath.row ];
    [[contentDataArray objectAtIndex:self.currentIndexPath.section] insertObject:tempArray atIndex:indexPath.row];
    
    [contentTableView insertRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
}
-(void)insertRowFromDown
{
    NSUInteger count = [datasource arrayDataForTopHeaderInTableView:self].count;
    NSMutableArray *tempArray = [[NSMutableArray alloc] init];
    for (int i=0; i < count; i++) {
    ...
    [[leftHeaderDataArray objectAtIndex:self.currentIndexPath.section] insertObject:tempArray atIndex:self.currentIndexPath.row+1];
    [[contentDataArray objectAtIndex:self.currentIndexPath.section] insertObject:tempArray atIndex:self.currentIndexPath.row + 1];
    NSIndexPath *indexPath = [NSIndexPath indexPathForRow:self.currentIndexPath.row+1  inSection:self.currentIndexPath.section];
    [contentTableView insertRowsAtIndexPaths:@[indexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
}
-(void)deleteThisRow
{
     [[leftHeaderDataArray objectAtIndex:self.currentIndexPath.section] removeObjectAtIndex:self.currentIndexPath.row];
     [[contentDataArray objectAtIndex:self.currentIndexPath.section] removeObjectAtIndex:self.currentIndexPath.row];
    
    [contentTableView deleteRowsAtIndexPaths:@[self.currentIndexPath] withRowAnimation:UITableViewRowAnimationAutomatic];
}
